...
router.put("/setting", permission({ level: ROLE.ADMIN }), async (ctx) => {
  ...
  if (config && systemConfig) {
    ...
    // Public registration
    if (config.publicRegister != null) systemConfig.publicRegister = Boolean(config.publicRegister);

    // Brand / basic info
    if (config.brandName != null) systemConfig.brandName = String(config.brandName);
    if (config.brandOwner != null) systemConfig.brandOwner = String(config.brandOwner);
    if (config.brandEmail != null) systemConfig.brandEmail = String(config.brandEmail);
    if (config.brandGithub != null) systemConfig.brandGithub = String(config.brandGithub);
    if (config.brandWebsite != null) systemConfig.brandWebsite = String(config.brandWebsite);
